...
- Bind component visuals to the semantic tokens in `src/tokens/semantic.css` and the typed Tailwind primitives needed to express their code implementation through the authoritative code-parity variable collection recorded by `library.tokenPolicy.componentVariableCollectionId` and `componentVariableIds`. Never select a variable by display name alone: duplicate names can exist in legacy collections. If a required code primitive is missing, add its typed variable, scope, resolved value, Web syntax, and exact ID to that allowlist before binding the component. Do not approximate a code token with a Cumulative value or copy a raw color into the component.
...
- Extend the component journal entry with the decision IDs, node identities, findings, fixes, and final result. Only then set `figma.review` to `status: "passed"`, the applicable registered standard from [`REVIEW-STANDARDS.md`](REVIEW-STANDARDS.md), `source-parity`, `adversarial`, and `design` passes, and that journal path as evidence.
