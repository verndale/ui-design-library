...
- `graphify-out/` is local, generated orientation data and stays gitignored, matching `ui-design-evidence`; the separate curated repository graph under `scripts/graph/data/` remains committed and authoritative for governed metadata and wiki history.
- Git events best-effort refresh the ignored local map: pre-commit covers local changes, post-merge covers pulls/merges, and branch-switch post-checkout covers incoming history changes. Graphify is orientation tooling, so an unavailable or failed refresh warns without blocking Git. `GRAPHIFY_SKIP_HOOK=1` explicitly skips the attempt.
- For code changes in this repository, use `pnpm graphify:sync` instead of the raw upstream update command. It bootstraps a missing local map with AST-only extraction, otherwise wraps `graphify update . --force` so branch/refactor deletions are reflected, and constrains extraction to a single worker. It is also the manual recovery command after a hook warning or a Git operation that does not fire these hooks.
